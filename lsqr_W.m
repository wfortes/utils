function [x,flag,relres,iter,resvec,lsvec,solvec] = lsqr(A,b,tol,maxit,M1,M2,x0,varargin)
%LSQR   LSQR Method.
...
%   $Revision: 1.6.4.9 $ $Date: 2009/11/16 22:27:13 $
...
[atype,afun,afcnstr] = iterchk_W(A);
...
if ((nargin >= 5) && ~isempty(M1))
    existM1 = 1;
    [m1type,m1fun,m1fcnstr] = iterchk_W(M1);
...
if ((nargin >= 6) && ~isempty(M2))
    existM2 = 1;
    [m2type,m2fun,m2fcnstr] = iterchk_W(M2);
...
if ~nKnown
    ...
    x = iterapp_W('mtimes',afun,atype,afcnstr,b,varargin{:},'transp');
    n = size(x,1);
end
...
u = b - iterapp_W('mtimes',afun,atype,afcnstr,x,varargin{:},'notransp');
beta = norm(u);
...
phibar = beta;
d = zeros(n,1);
v = iterapp_W('mtimes',afun,atype,afcnstr,u,varargin{:},'transp');
if existM2
    v = iterapp_W('mldivide',m2fun,m2type,m2fcnstr,v,varargin{:},'transp');
    if any(~isfinite(v))
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
if existM1
    v = iterapp_W('mldivide',m1fun,m1type,m1fcnstr,v,varargin{:},'transp');
    if any(~isfinite(v))
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
% solvec = zeros(maxit+1,1);   % by Wagner in May 10th 2010
solaux = 1;
sizex = size(x,1);
solvec(1:sizex,solaux) = x;    % (non-preconditioned ?)
...
for ii = 1 : maxit
    if existM1
        z = iterapp_W('mldivide',m1fun,m1type,m1fcnstr,v,varargin{:},'notransp');
        if any(~isfinite(z))
            flag = 2;
            iter = ii-1;
    ...
    if existM2
        z = iterapp_W('mldivide',m2fun,m2type,m2fcnstr,z,varargin{:},'notransp');
        if any(~isfinite(z))
            flag = 2;
            iter = ii-1;
            resvec = resvec(1:iter+1);
    ...
    u = iterapp_W('mtimes',afun,atype,afcnstr,z,varargin{:},'notransp') - alpha * u;
    beta = norm(u);
    u = u / beta;
    ...
    solaux = solaux + 1;         % by Wagner in May 10th 2010
    solvec(1:sizex,solaux) = x;  % same
    
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = iterapp_W('mtimes',afun,atype,afcnstr,u,varargin{:},'transp');
    if existM2
        vt = iterapp_W('mldivide',m2fun,m2type,m2fcnstr,vt,varargin{:},'transp');
        if any(~isfinite(vt))
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
    ...
    if existM1
        vt = iterapp_W('mldivide',m1fun,m1type,m1fcnstr,vt,varargin{:},'transp');
        if any(~isfinite(vt))
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
